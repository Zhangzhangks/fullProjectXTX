<template>
    <div class="cart">
        ...
        <div class="layer" v-if="validTotal > 0 && $route.path !== '/cart'">
            <div class="list">
                <div class="item" v-for="i in effectCount" :key="i">
                    <RouterLink :to="`/product/${i.id}`">
                        <img :src="i.picture" alt="">
                        <div class="center">
                            <p class="name ellipsis-2">{{ i.name }}</p>
                            <p class="attr ellipsis">{{ i.attrsText }}l</p>
                        </div>
                        <div class="right">
                            <p class="price">&yen;{{ (parseFloat(i.price) * i.count).toFixed(2) }}</p>
            ...
            <div class="foot">
                <div class="total">
                    ...
                    <p>&yen;{{ validTotalPrice.toFixed(2) }}</p>
                </div>
...
const { validTotalPrice, effectCount, validTotal } = storeToRefs(useCartStore())
const { findCart, deleteCart } = useCartStore()
onMounted(() => {
    findCart();
})
